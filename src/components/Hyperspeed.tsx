...
const Hyperspeed: React.FC<HyperspeedProps> = ({
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container: HTMLElement, options: HyperspeedOptions) {
        ...
        if (this.options.distortion == null) {
          this.options.distortion = 'turbulentDistortion';
        }
        ...
        this.targetCameraPos = new THREE.Vector3(0, 8, -5);
      ...
      update(delta: number) {
        let lerpPercentage = Math.exp(-(-60 * Math.log2(1 - 0.15)) * delta);
        ...
        if (fovChange !== 0) {
          this.camera.fov += fovChange * delta * 8;
          updateCamera = true;
        }

        this.targetCameraPos.x = this.mouse.x * 2;
        this.targetCameraPos.y = 8 + this.mouse.y * 1.5;
        ...
        const distortionObj = distortions[this.options.distortion];
        if (distortionObj && distortionObj.getJS) {
    ...
    class CarLights {
      ...
      constructor(webgl: App, options: HyperspeedOptions, colors: any, speed: [number, number], fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = options;
        this.colors = colors;
        this.speed = speed;
      ...
      init() {
        ...
        const distortionObj = distortions[options.distortion];
    ...
    (function () {
      ...
      options.distortion = options.distortion || 'turbulentDistortion';
